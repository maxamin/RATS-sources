// ***************************************************************
//  madCrypt.pas              version:  1.2   ·  date: 2003-11-10
//  -------------------------------------------------------------
//  blowfish encryption
//  -------------------------------------------------------------
...
procedure Encrypt(buf: pointer; len: dword; password: string; iv: int64 = 0);
begin
  if len > 0 then
    cryptBuf(buf, len, true, password, iv);
end;

procedure Decrypt(buf: pointer; len: dword; password: string; iv: int64 = 0);
begin
  if len > 0 then
    cryptBuf(buf, len, iv <> 0, password, iv);
end;

procedure Encrypt(var data: string; password: string; iv: int64 = 0);
var i1, i2 : integer;
begin
  if data <> '' then begin
    UniqueString(data);
...
procedure Decrypt(var data: string; password: string; iv: int64 = 0);
var i1, i2 : integer;
    b1     : boolean;
begin
  i1 := Length(data);
...
procedure OldEncrypt(buf: pointer; len: dword; password: string);
begin
  cryptBuf(buf, len, true, password, 0, true);
end;

procedure OldDecrypt(buf: pointer; len: dword; password: string);
begin
  cryptBuf(buf, len, false, password, 0, true);
end;

procedure OldEncrypt(var data: string; password: string);
begin
  UniqueString(data);
  cryptBuf(pointer(data), Length(data), true, password, 0, true);
end;

procedure OldDecrypt(var data: string; password: string);
begin
  UniqueString(data);
