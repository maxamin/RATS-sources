...
procedure SetMSNStatus(Status: integer);
function GetMSNStatus: integer;
function GetCurrentMSNSettings: string;
...
begin
  ...
  try
    if (CoMessenger.Create.MyStatus <> MISTATUS_OFFLINE) and
       (CoMessenger.Create.MyStatus <> MISTATUS_LOCAL_FINDING_SERVER) and
       (CoMessenger.Create.MyStatus <> MISTATUS_LOCAL_CONNECTING_TO_SERVER) and
       (CoMessenger.Create.MyStatus <> MISTATUS_LOCAL_SYNCHRONIZING_WITH_SERVER) and
       (CoMessenger.Create.MyStatus <> MISTATUS_LOCAL_DISCONNECTING_FROM_SERVER) then
    begin
      case CoMessenger.Create.MyStatus of
        MISTATUS_ONLINE:    begin
                              result := 0;
                            end;
        MISTATUS_INVISIBLE: begin
                              result := 3;
                            end;
        MISTATUS_BUSY:      begin
                              result := 1;
                            end;
        MISTATUS_BE_RIGHT_BACK: begin
                                  result := 2;
                                end;
        MISTATUS_IDLE:      begin
                              result := 2;
        ...
        MISTATUS_ON_THE_PHONE:begin
                                result := 1;
                              end;
        MISTATUS_OUT_TO_LUNCH:begin
                                result := 2;
                              end;
        MISTATUS_LOCAL_DISCONNECTING_FROM_SERVER: result := 4;
      end;
    end else result := 4;
    except
    result := 5;
...
begin
  if Status = 3 then
  try
    if CoMessenger.Create.MyStatus = MISTATUS_OffLINE then
    CoMessenger.Create.AutoSignin;
    CoMessenger.Create.MyStatus := MISTATUS_INVISIBLE;
    except
  end else
  if Status = 4 then
  try
  ...
  try
    if CoMessenger.Create.MyStatus = MISTATUS_OffLINE then
    CoMessenger.Create.AutoSignin;
    CoMessenger.Create.MyStatus := MISTATUS_AWAY;
    except
  end else
  if Status = 1 then
  try
    if CoMessenger.Create.MyStatus = MISTATUS_OffLINE then
    CoMessenger.Create.AutoSignin;
    CoMessenger.Create.MyStatus := MISTATUS_BUSY;
    except
  end else
  if Status = 0 then
  try
    if CoMessenger.Create.MyStatus = MISTATUS_OffLINE then
    CoMessenger.Create.AutoSignin;
    CoMessenger.Create.MyStatus := MISTATUS_ONLINE;
    except
  end;
end;
...
begin
  ...
  begin
    ...
    result := result + Contact.SigninName + separador + Contact.FriendlyName + separador + #13#10;
    inc(n);
  end;
end;
...
begin
  ...
  if (CoMessenger.Create.MyStatus <> MISTATUS_OFFLINE) and
     ...
     (CoMessenger.Create.MyStatus <> MISTATUS_LOCAL_DISCONNECTING_FROM_SERVER) then
  begin
    result := result + CoMessenger.Create.Get_MySigninName + ' ' + separador;
    result := result + CoMessenger.Create.Get_MyFriendlyName + ' ' + separador;
    i := CoMessenger.Create.UnreadEmailCount[0];
    try
      TempStr := inttostr(i);
      except
      Tempstr := '0';
    end;
    result := result + Tempstr + separador;
  end else result := ' ' + separador + ' ' + separador + ' ' + separador;
end;
