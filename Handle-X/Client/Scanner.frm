...
Private Sub ScannerGO()
    ...
    'Calculate TimeOut Amount
        PING_TIMEOUT = Val(Trim$(ScannerData(4)))
            ...
            'Ping IP First
             ...
             If Ping(CurrentIP, "Echo !", Echo) = 0 Then
                ...
                If (Not StopScan) Then
                     AddText Log(0), "[" & CurrentIP & "] Is Successfully Found ONLINE"
                    'Scan Ports Here
                     CurrentPort = FromPort
                     CurrentSock = 0
                     Do While (Not StopScan) And (CurrentPort <= ToPort)
                        If ThreadReady(CurrentSock) = True Then
                            ThreadReady(CurrentSock) = False
                            Timer(CurrentSock).Tag = CurrentPort 'PUSH Current Port
                            Scanner(CurrentSock).Tag = CurrentIP 'PUSH Current IP
                            Label1(9) = CurrentPort
                            CurrentSock = (CurrentSock + 1) Mod Threads 'Next Socket
                            CurrentPort = CurrentPort + 1
                            Scanner(CurrentSock).Close
                            Scanner(CurrentSock).Connect CurrentIP, CurrentPort
                            Timer(CurrentSock).Enabled = True
                            DoEvents
                        Else
                            CurrentSock = (CurrentSock + 1) Mod Threads 'Next Socket
                            DoEvents
...
Private Sub Speed_Click(Index As Integer)
    ScannerData(4).Enabled = (Speed(2).Value)
    ScannerData(4) = Speed(Index).Tag
End Sub
...
Private Sub ScannerData_LostFocus(Index As Integer)
    Select Case Index
        ...
        Case 4 'Time Out Value
                ScannerData(Index) = Val(Trim$(ScannerData(Index)))
                Speed(2).Tag = ScannerData(Index)
    End Select
End Sub
